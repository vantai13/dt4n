#!/usr/bin/env python3
"""RewardCalculator v3 cho TwinEnv — ham THUAN, test duoc khong can Mininet.
...
Trong do moi thanh phan chong mot benh:
    throughput   -> chong reward hacking "tat het link / khong phuc vu traffic"
    -c_act       -> chong spam action thua
    -c_step      -> ap luc "dung le me"
    +R_rec(t)    -> moc dich + thuong toc do (phuc hoi som duoc nhieu hon)

Muc tieu thiet ke: throughput-centric. Util nam trong STATE de agent dinh vi
nghen, nhung khong phat truc tiep trong reward vi TCP baseline co the bao hoa
link va lam util cao ngay ca khi mang dang phuc vu tot.
"""
...
def compute_reward(throughput_norm, action_is_noop,
