...
sudo "$PYTHON_BIN" -m mininet.run_sync_v7 \
  ...
  --core-sigma 0.10 \
  --edge-sigma 0.03 \
  --offered-out results/SMOKE/phase-20/rho_offered_smoke.csv \
  --measured-out results/SMOKE/phase-20/rho_measured_smoke.csv \
  --meta-out results/SMOKE/phase-20/rho_trace_smoke_meta.json \
  --flow-log-dir results/SMOKE/phase-20/flow_logs_smoke \
  2>&1 | tee logs/phase20_tau_smoke.log

echo "[4/5] analyze offered smoke trace"
"$PYTHON_BIN" -m measurements.measure_tau \
  --input results/SMOKE/phase-20/rho_offered_smoke.csv \
  --core-sigma-target 0.10 \
  --edge-sigma-target 0.03 \
  --out results/SMOKE/phase-20/tau_summary_smoke.json \
...
"$PYTHON_BIN" -m measurements.compare_estimators \
  --offered results/SMOKE/phase-20/rho_offered_smoke.csv \
  --measured results/SMOKE/phase-20/rho_measured_smoke.csv \
  --out results/SMOKE/phase-20/estimator_compare_smoke.json \
  2>&1 | tee logs/phase20_tau_smoke_compare.log

echo "OK: smoke outputs are in results/SUPERSEDED/phase-20/ and logs/"
