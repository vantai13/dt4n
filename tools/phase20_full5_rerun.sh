...
sudo "$PYTHON_BIN" -m mininet.run_sync_v7 \
  ...
  --edge-sigma 0.03 \
  --seed 0 \
  --offered-out results/phase-20/rho_offered_10ms.csv \
  --measured-out results/phase-20/rho_measured_10ms.csv \
  --meta-out results/phase-20/rho_trace_10ms_meta.json \
  --flow-log-dir results/phase-20/flow_logs_10ms \
...
sudo "$PYTHON_BIN" -m mininet.run_sync_v7 \
  ...
  --edge-sigma 0.03 \
  --seed 0 \
  --offered-out results/phase-20/rho_offered_2ms.csv \
  --measured-out results/phase-20/rho_measured_2ms.csv \
  --meta-out results/phase-20/rho_trace_2ms_meta.json \
  --flow-log-dir results/phase-20/flow_logs_2ms \
...
for S in 0 1 2 3 4; do
  ...
  META="results/phase-20/rho_trace_long_meta.json"
  FLOW_DIR="results/phase-20/flow_logs_long"
  if [[ "$S" != "0" ]]; then
    OFFERED="results/phase-20/rho_offered_long_s${S}.csv"
    MEASURED="results/phase-20/rho_measured_long_s${S}.csv"
    META="results/phase-20/rho_trace_long_s${S}_meta.json"
    FLOW_DIR="results/phase-20/flow_logs_long_s${S}"
  fi
  sudo "$PYTHON_BIN" -m mininet.run_sync_v7 \
    --traffic v7 \
    --duration 1800 \
...
SUMMARY_INPUTS="results/phase-20/decision_error_trace_s0.json,results/phase-20/decision_error_trace_s1.json,results/phase-20/decision_error_trace_s2.json,results/phase-20/decision_error_trace_s3.json,results/phase-20/decision_error_trace_s4.json"
...
"$PYTHON_BIN" -m measurements.summarize_decision_error_replicates \
  --inputs "$SUMMARY_INPUTS" \
  --run-seed 100 \
  --out results/phase-20/between_trace_summary.json
"$PYTHON_BIN" -m measurements.summarize_decision_error_replicates \
  --inputs "$SUMMARY_INPUTS" \
  --run-seed 100 \
  --out results/phase-20/between_trace_summary_n5.json
...
echo "Main outputs:"
echo "  results/phase-20/between_trace_summary_n5.json"
echo "  results/phase-20/core_load_diagnostic_n5.json"
echo "  results/phase-20/block_crossing_diagnostic_n5.json"
echo "  results/phase-20/decision_error_measured_fixed_replicates_summary.json"
echo "  results/phase-20/measured_fixed_crosscheck_diagnostic_n5.json"
