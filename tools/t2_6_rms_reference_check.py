...
def main(argv=None):
    ...
    out['validity'] = pending_validity('estimand_reference_audit', 'Historical margin/cost reference uses legacy age, not the measured-v7 all-action estimand; retain until its certificate axis is registered.')
    Path(a.out).write_text(json.dumps(out,indent=2)+'\n')
    if a.csv_out:
        dec.to_csv(a.csv_out,index=False)
