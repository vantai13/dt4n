...
HOST_ALIAS="${HOST_ALIAS:-dt4n-aws}"
...
if [ -z "$public_ip" ]; then
  echo "ERROR: Chua co EC2 public IP. Hay chay terraform apply truoc, hoac truyen IP vao script:" >&2
  echo "  ./scripts/setup-vscode-ssh-from-terraform.sh EC2_PUBLIC_IP" >&2
  exit 1
fi
...
powershell.exe -NoProfile -ExecutionPolicy Bypass \
  ...
  -SourceKey "$(wslpath -w "$ROOT_DIR/dt4n-aws.pem")"
