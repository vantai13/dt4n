...
usage() {
  cat <<'EOF'
Usage:
  ./scripts/sync-project.sh ubuntu@EC2_IP [~/.ssh/dt4n-aws] [/home/ubuntu/dt4n]

Dong bo source DT4N local len EC2 bang rsync.
...
#   INCLUDE_RUNS=1 ./scripts/sync-project.sh ubuntu@IP ~/.ssh/dt4n-aws
