param(
    [string]$HostName = "",
    [string]$HostAlias = "dt4n-gcp",
    [string]$User = "ubuntu",
    [string]$SourceKey = "",
    [string]$TargetKeyName = "dt4n-gcp.pem"
)
...
if ([string]::IsNullOrWhiteSpace($SourceKey)) {
    ...
    $SourceKey = Join-Path (Split-Path -Parent $ScriptDir) "dt4n-gcp.pem"
}
...
if ([string]::IsNullOrWhiteSpace($HostName)) {
    $HostName = "REPLACE_WITH_GCE_PUBLIC_IP"
}
...
if ($HostName -eq "REPLACE_WITH_GCE_PUBLIC_IP") {
    Write-Host "Sau khi terraform apply co IP, chay lai script nay voi -HostName GCE_PUBLIC_IP."
} else {
