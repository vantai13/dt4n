...
| G23-297 | 23.24 | PASS | `M-233` tren CALIB (499798 hang, truc DO): tang 1 cho `{P2, P4}` (`P_calib(a*)` = 0.0 va 0.006567 < 0.05); tang 2(a) loai P4 vi `P_calib(a_twin=P4)` = 0.010384 > 0. Tap CAT = **{P2}**, dung dai da ky. Chan quy tac ba: **6.0024e-06**. Doi chieu winner's curse: tren TEST 23.7 `P(a*=P2)` = 1.4e-05, tren CALIB = **0.0** |
| G23-298 | 23.24 | PASS | `M-234`(a) ti so `q_hat(K=3,alpha/2)/q_hat(K=4,alpha/3)` = **0.923259** tren 4 bin x 2 slot chung, trong dai [0.88, 0.94]. (b) `Delta acceptance(S1 vs S0)` = **+0.039508**, trong dai [+0.01, +0.05]. (c) [MO TA] `Delta acceptance(S2 vs S0)` = +0.124015. Huong lech du bao SAI: A074 muc 5 doan ti so nam DUOI 0.921016, do duoc **tren** (+0.24%); dai van dat |
| G23-299 | 23.24 | PASS | `M-235` `budget_share(S1)` = **1.003850** >= 0.90. Nhanh (i) CHI cat P2 giu nguyen alpha cua S0: `Delta acceptance` = **-0.000300**, tuc AM. Nhanh (ii) CHI noi `alpha_each`: **+0.039660**. Tuong tac +0.000148. Toan bo loi ich den tu KENH NGAN SACH; viec don khong gian hanh dong tu no KHONG mua duoc acceptance. S2: budget_share 0.900613, constraint_share 0.097725 |
| G23-300 | 23.24 | FAIL | `NC-23.24-1` ban sua (`A074b` muc 4). Ve (iii) VE CHAN DAT: `P_calib(a_twin=P2)`=0, `P_calib(a_twin=P3)`=0.376902>0.05. Ve (ii) **KHONG DAT**: `Delta err\|accept (cat P3 vs cat P2)` = **-0.091538**, dai doi >= +0.02 -- cat duong SONG lam `err\|accept` TOT len (0.012256 vs 0.103794), khong xau di. `anchor_moved_rate` 0.374783 (P3) vs 0.000000 (P2). Ve (i) da BO truoc khi chay. Kich ban **K2** (`A074` muc 6): cau K1 KHONG duoc phat bieu, `L43` KHONG dong duoc. Xem `L131` |
