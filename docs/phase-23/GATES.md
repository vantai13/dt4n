...
| G23-297 | 23.24 | PASS | `M-233` tren CALIB (499798 hang, truc DO, `w_loss=5000`, ho so AoI `U0`): tang 1 cho `{P2, P4}` (`P_calib(a*)` = 0.0 va 0.007645 < 0.05); tang 2(a) loai P4 vi `P_calib(a_twin=P4)` = 0.011985 > 0. Tap CAT = **{P2}**, dung dai da ky. Chan quy tac ba: **6.0024e-06**. Doi chieu winner's curse: tren TEST 23.7 `P(a*=P2)` = 1.4e-05, tren CALIB = **0.0**. Thi hanh LAN HAI sau `A075` (lan dau VOID vi `w_loss` sai); phan quyet khong doi |
...
| G23-301 | 23.24b | DEBT | dinh nghia o `A076` muc 8: RAW 426 MiB da nen thanh `~/dt4n-raw-phase23-aoi-20260827.tar.gz` (56 MiB), SHA256 = **a88c669f741a308f067033f6f1ed370f696b6fee6b9d0e2132cf6ea3542bd7c1**. DOI Zenodo CHUA CO -- can tai khoan cua tac gia. SHA256 tra loi "file toi dung CHINH LA file do"; DOI tra loi "file do ton tai tu ngay do". Can CA HAI, nen gate GIU DEBT cho toi khi co DOI. `G23-74` van mo |
| G23-302 | 23.24b | PASS | R2 `rho` tu SO SACH bo sinh tai tren 30 run / 8 link. `rho_gen / rho_target` = uA 0.974, uB 0.991, ac 0.982, ad 0.997, bc 0.978, bd 0.988, vC 0.976, vD 0.993. uA/uB CO tai va don dieu tang theo `rho_bar` (0.614 -> 0.884). **NHAN `[MO TA -- DA DO TRUOC KHI KY]`** (`A076` muc 2): khong duoc dem vao so diem du doan |
| G23-303 | 23.24b | PASS | **`CSV_CLEAN`, 8/8 link.** `M-236` `zero_share(uA)` = **0.001169** < 0.05 HIT; `M-237` `zero_share(uB)` = **0.000556** < 0.05 HIT; `M-238` max 6 link con lai = 0.001669 (`vD`) HIT; `M-240` 8/8 CLEAN HIT. Doi chieu: cung 30 run do, nhanh DITTO cho `zero_share(uA)` = **0.980595**. Hai nhac cu, mot hong mot sach. Kich ban **`K1`** thi hanh: `rho_measured_*.csv` la nguon HOP LE cho Lesson 23.25 |
...
| G23-307 | 23.25 | PASS | `M-242` `sum k^2` = **5.0000**; `M-243` ti so `Var(m)` tai `omega=1` cap KE = **1.70711** (4/4 cap, dong nhat); `M-244` cap CHEO = **1.94281** (2/2, dong nhat). Ba so deu khop giai tich tu topology. Nhan [TAT DINH] -- kiem CAI DAT, khong phai phat hien (tien le `M-193`/`M-200`) |
| G23-308 | 23.25 | PASS | `M-245` `omega_hat` = **+0.0852** trong dai [0.00, 0.15]; `omega_hat_corrected` = **-0.0828**; `omega_hat_deattenuated` = -0.0930. `M-246` `b_hat` (16 cap NULL) = **+0.1097** trong dai [-0.05, +0.15]. `M-247` `n_eff/cap` = **393** trong dai [30, 4000]. CI95 block bootstrap = [-0.0314, +0.0475]. **CANH BAO:** block 149 mau (29.8 s) = 1.08 x `tau_system` (27.67 s), duoi nguong 3.0 -> CI la CAN DUOI cua do rong that va `n_eff` la CAN TREN (`L139`) |
...
| G23-312 | 23.25b | ADJUDICATED | Phan xu H1 (confound chung diem cuoi) vs H2 (chuoi ngan) roi vao KHE HO cua ba kich ban da ky. `M-252` `sd(r(uA,uB))` qua 15 run = **0.2839** (< 0.30, ve H1 DAT); `M-253` so run AM = **1/15** (ve H1 DAT); `M-254` `\|r_offered(uA,uB)\|` = **0.1725** MISS (dai H1 doi < 0.15, dai H2 doi ~ +0.6 -- ngoai CA HAI); `M-255` doi chieu `sd(r(ac,ad))` = **0.1610** HIT. `K1` la LIEN KET VA nen KHONG dat; `K2`/`K3` cung khong. Ap XU LY BAO THU cua `K3`. Test B van mang thong tin: `r_offered` = +0.1725 / **-0.1832** trong khi `r_do` = +0.5986 / +0.6376, nen gia thuyet THIET KE BO SINH bi BAC BO. Xem `L145` |
| G23-313 | 23.25b | PASS | `T7_null_audit` day du. Tap NULL KHONG dong nhat: fast-fast +0.0163 (n=6), slow-fast +0.0527 (n=8), slow-slow **+0.6181** (n=2); spread 0.6018 vs sd gop 0.2104; ngoai lai `vC-vD`, `uA-uB`, `bd-vC`. `n_eff` theo cap = **32.5 .. 44.9** (0/12 cap la fast-fast), `sum(w*k^2)` = **175.70**, `sd` dung = **0.0754**, CI95 **[-0.0658, +0.2299]** -- rong gap **3.62 lan** bootstrap. `omega_hat` nam NGOAI CI95 cua chinh no -> bootstrap LECH VI TRI (`L143`). `NC-25b-2` PASS: `T0`..`T6` BIT-FOR-BIT khong doi |
| G23-314 | 23.25b | PASS | `M-256` `SNR_dec` trung vi khi BO hai cap ngoai lai = **0.3041** (day du 0.3752), trong dai da ky [0.28, 0.42]. `M-257` quyet dinh **KHONG DOI**: van `D3_OPEN_23_26_REDUCED_HIGHEST_SNR_CELL_ONLY`. Quyet dinh `D3` cua Lesson 23.25 BEN VUNG truoc viec bo hai phan tu ma tran chi phoi 85-111% cua `Var(m)` |
| G23-315 | 23.25c | PASS | In du 28 cap. Trong tap `k>0`, lop `k=0.7071` co **8/8** cap chung host; lop `k=0.5` co **0/4**. Cong tuyen hoan hao -> Lesson 23.25 la doi chung am, khong nhan dang path-coupling |
| G23-316 | 23.25c | PASS | Bartlett ACF do duoc tren 28 cap: fast-fast **660.3--871.0**, slow-fast **786.7--1784.5**, slow-slow **832.6--1537.7**. Hai du doan M-258 [150,500] va M-259 [40,150] deu MISS cao; gate PASS vi phep do 28 cap hoan tat va bac bo `n_eff=32.5--44.9` tu max-tau |
| G23-317 | 23.25c | PASS | `PC-25c-1` ma tran gia omega=0.5 thu hoi **0.520813** trong [0.45,0.55]. `NC-25c-2` ma tran don vi thu hoi omega=0; sd khop `(X'WX)^-1` voi sai so 0 |
| G23-318 | 23.25c | ADJUDICATED | M1: omega **-0.1022 +/-0.0173**, chi2/dof **46.478**; M3: omega **+0.0391 +/-0.0179**, `host_x_slow=+0.6895 +/-0.0221`, chi2/dof **9.407**. M-260/M-262 HIT; M-261/M-263/M-264 MISS. Ca M1 lan M3 thieu bien; khong CI nao duoc doc nhu uoc luong omega vat ly |
| G23-319 | 23.25c | ADJUDICATED | Probe: uA-uB offered/measured/shortfall = **+0.1736/+0.5986/+0.9020**; vC-vD = **-0.1840/+0.6376/+0.9612**. Endpoint contention duoc ho tro manh, generator/RNG bi bac bo. Nhanh tong **K3** vi M3 chi2/dof MISS; khong noi long phep VA da ky |
| G23-320 | 23.25c | PASS | `NC-25c-1`: T0..T7 canonical SHA256 cu/moi cung bang `b8e16c7...234fff5c`, `canonical_diff_count=0`. Ket qua moi chi them T8 |
| G23-321 | 23.25d | PASS | Moi WLS T8 in `S`, `sd_scaled`, `t_scaled`, `ci95_scaled`. M1 S=**6.817**, omega=-0.1022 +/-**0.1180**, CI95 [-0.333,+0.129]; M3 S=**3.067**, omega=+0.0391 +/-**0.0549**, CI95 [-0.068,+0.147]. Day la quasi-WLS diagnostic, khong sua model sai |
| G23-322 | 23.25d | PASS | M3 va M1 bo `{uA-uB,vC-vD}` trung: delta b=-6.94e-18, delta omega=+3.47e-17. Bac thang 28/26/24 cap: chi2/dof **46.478/9.709/2.445**, omega **-0.1022/+0.0391/+0.0276**, sd_scaled **0.1180/0.0557/0.0299** |
| G23-323 | 23.25d | FAIL | Bon link bien fit hop le: lambda uA/uB/vC/vD = **0.619/0.506/0.642/0.694**, co bootstrap CI95; tau=9.39--13.04 s. Bon link loi co slope am nhung intercept signal fraction **1.036--1.194 >1**, nen invalid va khong co CI day du. Gate doi lambda/tau/CI ca 8 link, chi dat 4/8 |
| G23-324 | 23.25d | ADJUDICATED | N doi tat ca fit hop le + median edge signal<=0.5; T doi min all>=0.85. Tat ca fit hop le=FALSE, median edge signal=**0.3696**. Thi hanh dung nhanh phu kin **DEFAULT_MIXED_OR_INVALID**, khong chon N gan nhat |
| G23-325 | 23.25d | ADJUDICATED | Fit nugget cua 6 cost-margin deu invalid (signal fraction raw 1.050--1.156), nen `SNR_corrected=None`, quyet dinh **UNDECIDED**. D3 measured=0.3752 giu lich su nhung TREO; khong tu dong doi thanh D2 hay xac nhan D3 |
| G23-326 | 23.25d | PASS | May kiem admissibility phat hien M1 omega=-0.1022 ngoai [0,1], doi scenario thanh **K3_DEFAULT_INADMISSIBLE_DISCLOSED**; scaled CI chua 0 khong bien point estimate thanh admissible. Unit test chung minh nhanh invalid phai tu to cao |
| G23-327 | 23.25e | PASS | Contrast 2x2 in du 28 cap. O khong chung host: mean r(k=0.5)=**+0.03642** (n=4), mean r(k=0)=**+0.04505** (n=10), nen omega mo ta = **-0.01728**. Hai o k=0.5/chung-host va k=0.7071/khong-chung-host rong; khong goi contrast nay la causal clean estimate |
| G23-328 | 23.25e | PASS | T5b dung `D R D` voi sigma target edge/core=0.03/0.10. Sau ratio measured/identity = **0.8803--0.9577** (M-270 HIT); moc omega=1 = **1.389233** cho 4 cap ke (M-271 HIT), **1.719092** cho 2 cap cheo. T5 correlation cu giu lich su nhung khong duoc trich thay covariance |
| G23-329 | 23.25e | PASS | T9 co dung **2** vi pham tran independent-residual (M-272 HIT): uA-uB r=+0.5986 > 0.4338 (**1.380x**); vC-vD +0.6376 > 0.3308 (**1.927x**). Ket luan model-free: residual lag-0 cross-correlated; quy no cho host can probe shortfall doc lap |
| G23-330 | 23.25e | FAIL | M-273a MISS: Spearman(log pair-process dose,r)=**+0.6571**, p=0.1562, duoi nguong +0.80 tren 6 cap. Dinh nghia total-endpoint cho **+0.6000**, p=0.2080. Co che dose-response bi bac theo dai; host shortfall van la artifact dung cu, khong thanh quan he nhan qua tong host load |
| G23-331 | 23.25e | ADJUDICATED | M-274 sd-ratio MISS: median core clean@0.960 = **0.6992** >0.60. Monitor p(rho_measured>0.99)=0.4741--0.5042 tung duoc goi la tran cung, nhung **G23-335/K09 rut dien giai nay**: 0.99 thap hon tran 1.009410; p_censored dung tai 0.960 chi 0.0355. Giu dong de bao toan lich su, khong trich lam bang chung censoring |
| G23-332 | 23.25e | ADJUDICATED | D3 measured=0.3752 chi giu lam pilot budget; **D1/D2/D3 corrected = UNDECIDED**. Nhanh T11 saturation da bi G23-335 supersede, nhung T9 residual cross-correlated van du bac lap luan "SNR measured la can duoi vo dieu kien"; T12 sau do cung FAIL doi chung (`G23-334`). Khong thi hanh D3 nhu dinh ly |
| G23-333 | 23.25e | PASS | Audit ghi artifact rieng. SHA256 canonical T0..T8 truoc/sau cung **188472781e66848a33cc7e55d040b397362a77f521308e2f3e8c642f28298a49**; output khong alias corr/nugget source. T0..T8 khong doi |
| G23-334 | 23.25f | FAIL | Doc doi chung truoc: M-276 R(0.700)=**1.1344** ngoai [0.95,1.05] trong khi p_censored=0.0004. Phan xu da ky **NEGATIVE_CONTROL_FAILED_STOP**; M-275 R(0.960)=1.3520 KHONG duoc doc va cell script chon KHONG actionable. Quet post-hoc 20 pha aggregate chi cho 1.1311--1.1351, nen khong sua offset de ep HIT |
| G23-335 | 23.25f | PASS | Tran TX do duoc K09=**1.0094102536** tren 49 link-run (MAD 0.00003562); nguong censoring=**1.0043632023**. p_censored theo cell=0.0004/0.0117/0.0307/0.0361/0.0355. M-277 clip offered tai 0.960=**0.1302** (<0.15 nhung khong nho); M-278 Spearman(p,R)=**+0.10**, p=0.873. Nguong cu 0.99 bi rut |
| G23-336 | 23.25f | PASS | T0..T11 canonical SHA256 truoc/sau = **9b82306376688f275a9e31a5a926c56a7d14f3b9a23704723c441a160339e3fb**. Whole-file SHA256 link/final audit cung giu `6a753c...e8e291` / `64cdf9...ce78`. T12 la artifact append-only rieng |
