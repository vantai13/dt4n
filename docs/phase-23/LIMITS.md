...
| L80 | ★ Tien de cua `L51` SAI: digest lich su KHONG mat. Chung nam trong `provenance.inputs` cua `results/SUPERSEDED/phase-23/eight_cell_sweep_U3_measured_v7.json` (git_hash `05b597f5`). Doi chieu duoc: 3 file con song KHOP digest goc (`calib_set_v3`, `_h2_0.700`, `_poisson_0.850`), 1 file KHAC (`_poisson_0.700`: lich su `ec49deb8...`, tren dia `2267423d...`), 5 mat | do 2026-08-24; `results/RAW/phase-22/SURVIVING_CALIB_DIGESTS.json` | 7 parquet Phase 22 (468 MB) la BAN DUY NHAT: khong trong git, `git_tracked=0/7`. DA ghim digest. `_poisson_0.700` phai bi CAM tai dung lam moc doi chung -- do dung cai bay `L51` canh bao. `truth_table.parquet` + `sla_calibration.json` doi chieu = KHOP, nen chung xac minh duoc |
