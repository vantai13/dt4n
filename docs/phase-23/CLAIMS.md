...
| CL-14 | Tuong quan tai THEO DUONG khong tac dong len do dung quyet dinh mot cach DOC LAP: no phong dai PHUONG SAI cua bien quyet dinh, nen tac dong cua no TI LE voi suc manh cua prior tinh. Tren testbed hien tai `omega_hat` = +0.0852 (can tren) va `SNR_dec` trung vi 0.3752, cho du bao `err` tang **2.96%** (cap KE) den **3.48%** (cap CHEO) khi `omega` di tu 0 len 1 | `G23-308` `M-245` `omega_hat` = +0.0852, `b_hat` = +0.1097, CI95 [-0.0314, +0.0475]; `G23-311` `M-250` `SNR_dec` = 0.3752 (0.1115--0.9690), `M-251` ti so 1.0296 / 1.0348; dai so Sheppard `A077` muc 2b | (1) DU BAO CO CHE, chua duoc kiem TRUC TIEP bang phep quet `omega` tren testbed -- nang len phat bieu THUC NGHIEM phai qua Lesson 23.26. (2) Khi `E[m] = 0` thi `err = arccos(r)/pi`, BAT BIEN voi `omega`; nen phat bieu nay CHI dung khi `SNR_dec > 0`, va do lon phu thuoc truc `swing` cua 23.27 -- hai truc KHONG doc lap. (3) `L139`: CI la CAN DUOI. (4) `L140`: `omega_hat` la CAN TREN o thang ms. (5) `L141`: `mode="poisson"` la anh xa, chua do `c_a`/`c_s`. (6) `L138`: `omega=1` la vat ly DA CHUAN HOA. **(7) ★ CAP NHAT `A078`:** rang buoc SO la `|omega| <= 0.15`, KHONG phai `omega = -0.083 +- 0.04`. `n_eff` thuc theo cap la 32.5-44.9 (khong phai 393) vi MOI cap co `k>0` deu chua mot link BIEN, nen `sd` dung = 0.0754 va CI95 = +-0.1479 -- rong gap 3.8 lan. DAU cua `omega_hat` KHONG xac dinh duoc. **(8) ★ `Var(m)_do/don_vi = 0.54-0.71` KHONG duoc dung lam bang chung cho `CL-14`**: 85-111% cua no den tu DUNG HAI phan tu `r(uA,uB)`=+0.599 va `r(vC,vD)`=+0.638, va co che cua chung (chung DIEM CUOI) NGUOC DAU voi co che cua `omega` (chung DUONG). `G23-312` phan xu roi vao KHE HO (`L145`); gia thuyet THIET KE BO SINH bi bac bo nhung H1 vs H2 chua nga ngu |
